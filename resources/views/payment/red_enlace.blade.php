    ...
    <div class="container-fluid container-fluid-90 margin-top-85 min-height">
        <div class="row">
            <div class="col-md-8 col-sm-8 col-xs-12 mb-5 main-panel p-5 border rounded">
                <div class="pb-3 m-0 text-24 font-weight-700">
                    {{ trans('messages.payment.payment') }} 
                    {{ trans('messages.payment.redenlace') }}
                </div>
                <form action="{{ URL::to('payments/red_enlace-request') }}" method="post" id="payment-form">
                    {{ csrf_field() }}
                    <div class="form-row p-0 m-0">
                        <label for="card-element">
                            {{ trans('messages.payment_red_enlace.red_enlace_payment') }}
                        </label>
                        <div id="card-element">
                    ...
                    <div class="form-group">
                        <label for="proof">{{ trans('messages.proof.proof_of_payment') }}<span style="color:red;">*</span></label>
                        <input type="file" class="form-control-file" id="proof" accept="image/*" multiple required>
                    </div>
                    <div class="form-group mt-5">
                        <div class="col-sm-8 p-0">
                            <button class="btn vbtn-outline-success text-16 font-weight-700 pl-5 pr-5 pt-3 pb-3"
                                id="stripe_btn"><i class="spinner fa fa-spinner fa-spin d-none"></i>
                                {{ trans('messages.payment_red_enlace.submit_payment') }}</button>
                        </div>
                    </div>
                </form>
            </div>
                ...
                <div class="card p-3">
                    ...
                    <div class="card-body p-2">
                        ...
                        <div class="border p-4 mt-3">
                            ...
                            <div class="d-flex justify-content-between text-16">
                                <div>
                                    <p class="pl-4">{!! moneyFormat($result->property_price->currency->symbol, $price_list->property_price) !!} x {{ $nights }}
                    ...
                    <div class="card-body text-16">
                        <p class="exfont">
                            ...
                            {{ trans('messages.payment.exchange_rate_booking') }} {!! $currencyDefault->org_symbol !!} 1 to
                            {!! moneyFormat($result->property_price->currency->org_symbol, $price_rate) !!} {!! $result->property_price->currency_code !!} (
