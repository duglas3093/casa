					...
					<div class="col-md-12 mt-5">
						<h3 class="text-20 font-weight-600">{{trans('messages.account_preference.my_wallet_balance')}}
						    <?php if($walletBalance->balance < 0) { ?>
						        <span class="vbadge-success p-2 button-rounded-4"> {{$default_currency->code }} {{ $walletBalance->balance }} </span>

						    <?php } else { ?>
						        <span class="Accepted p-2 button-rounded-4"> {{$default_currency->code }} {{ $walletBalance->balance }} </span>
						    <?php } ?>
						</h3>
					</div>
