...
$(document.body).on('click', '.date-package-modal', function(){
    $('#model-message').html("");
        ...
        var maxday = $(this).attr('data-maxday');
...
$(document.body).on('submit', "#dtpc_form", function(e){
  ...
  if(start_date != '' && end_date != '' && price != '' && status != ''){
    ...
    $.ajax({
        type: "POST",
        url: url,
        data: { "_token":  $('meta[name="csrf-token"]').attr('content'),'start_date':start_date, 'end_date':end_date, 'price':price, 'min_stay': min_stay,'max_stay': max_stay, 'status':status},
        success: function(msg) {
           //if(msg.status){
