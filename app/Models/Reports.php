...
class Reports extends Model
{
    ...
    public function getIncomes($startDate, $endDate, $propertyId = '')
    {
        ...
        $endDate     = $this->helper->y_m_d_convert($endDate);
        if ($propertyId == '') {
            $incomes     = Bookings::whereBetween(DB::raw('date(updated_at)'), [$startDate, $endDate])->where(['cancelled_at' => null, 'cancelled_by' => null,'status' =>'Accepted'])->select(['total', 'currency_code', 'id'])->get();

            foreach ($incomes as $income) {
                $totalIncome += $income->amounts;
            }
        } else {
            $incomes     = Bookings::whereBetween(DB::raw('date(updated_at)'), [$startDate, $endDate])->where(['property_id' => $propertyId, 'cancelled_at' => null, 'cancelled_by' => null,'status' =>'Accepted'])->select(['total', 'currency_code', 'id'])->get();
            foreach ($incomes as $income) {
                $totalIncome += $income->amounts;
            }
        }
